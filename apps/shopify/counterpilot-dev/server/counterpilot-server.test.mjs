...
import { calculateAppProxySignature, createCounterpilotServer } from "./counterpilot-server.mjs";
...
async function merchantAction(baseUrl, transactionId, action, payload, headers = {}) {
...
test("POST /counterpilot/offers persists an offer_submitted event and returns only safe fields", async () => {
  await withServer(async ({ baseUrl, dataDir }) => {
    ...
    assert.equal(events[0].operational_refs.product_ref, "gid://shopify/Product/123");
...
test("POST /apps/counterpilot/offers verifies app-proxy signatures when a secret is configured", async () => {
  ...
  await withServer(async ({ baseUrl }) => {
    ...
    params.set("signature", calculateAppProxySignature(params, appProxySecret));
    const signed = await postJson(`${baseUrl}/apps/counterpilot/offers?${params}`, validOffer());
...
test("GET /counterpilot/merchant/offers lists current offer states without raw buyer or Shopify references", async () => {
  await withServer(async ({ baseUrl }) => {
    ...
    const { response } = await merchantAction(baseUrl, transactionId, "counter", {
    ...
    const inboxResponse = await fetch(`${baseUrl}/counterpilot/merchant/offers?shop=counterpilot-dev.myshopify.com`);
...
test("submitted offers can be accepted manually", async () => {
  await withServer(async ({ baseUrl, dataDir }) => {
    const transactionId = await submitOffer(baseUrl);
    const { response, body } = await merchantAction(baseUrl, transactionId, "accept", {
      store_id: "counterpilot-dev.myshopify.com"
    });
    assert.equal(response.status, 200);
    ...
    const events = await readOfferEvents(dataDir);
    assert.equal(events.map((event) => event.event_type).join(","), "offer_submitted,merchant_accepted");
...
test("submitted offers can be countered manually", async () => {
  await withServer(async ({ baseUrl, dataDir }) => {
    const transactionId = await submitOffer(baseUrl);
    const { response, body } = await merchantAction(baseUrl, transactionId, "counter", {
      store_id: "counterpilot-dev.myshopify.com",
...
test("submitted offers can be declined manually", async () => {
  await withServer(async ({ baseUrl, dataDir }) => {
    const transactionId = await submitOffer(baseUrl);
    const { response, body } = await merchantAction(baseUrl, transactionId, "decline", {
      store_id: "counterpilot-dev.myshopify.com"
    });
    assert.equal(response.status, 200);
...
test("merchant actions reject invalid transitions and unknown transactions", async () => {
  await withServer(async ({ baseUrl }) => {
    ...
    const declined = await merchantAction(baseUrl, declinedTransactionId, "decline", {
      store_id: "counterpilot-dev.myshopify.com"
    });
    assert.equal(declined.response.status, 200);
    const declinedCounter = await merchantAction(baseUrl, declinedTransactionId, "counter", {
      store_id: "counterpilot-dev.myshopify.com",
      counter_amount: "625.00",
      currency: "USD"
    });
    ...
    const accepted = await merchantAction(baseUrl, acceptedTransactionId, "accept", {
      store_id: "counterpilot-dev.myshopify.com"
    });
    assert.equal(accepted.response.status, 200);
    const acceptedCounter = await merchantAction(baseUrl, acceptedTransactionId, "counter", {
      store_id: "counterpilot-dev.myshopify.com",
      counter_amount: "625.00",
      currency: "USD"
    });
    ...
    const unknown = await merchantAction(baseUrl, "cp_offer_missing", "decline", {
      store_id: "counterpilot-dev.myshopify.com"
    });
...
test("counter actions require positive same-currency counter amounts", async () => {
  await withServer(async ({ baseUrl }) => {
    for (const payload of [
      { store_id: "counterpilot-dev.myshopify.com", counter_amount: "0", currency: "USD" },
      { store_id: "counterpilot-dev.myshopify.com", counter_amount: "10.999", currency: "USD" },
      { store_id: "counterpilot-dev.myshopify.com", counter_amount: "625.00", currency: "EUR" }
    ]) {
      const transactionId = await submitOffer(baseUrl);
      const result = await merchantAction(baseUrl, transactionId, "counter", payload);
...
test("merchant actions reject notes, checkout URLs, phone numbers, and unexpected fields", async () => {
  await withServer(async ({ baseUrl }) => {
    for (const payload of [
      ...
      { store_id: "counterpilot-dev.myshopify.com", checkout_url: "https://example.com/checkout" },
      { store_id: "counterpilot-dev.myshopify.com", phone: "555-0100" },
      { store_id: "counterpilot-dev.myshopify.com", access_token: "secret-token" },
      { store_id: "counterpilot-dev.myshopify.com", unsupported_field: "not accepted" }
    ]) {
      const transactionId = await submitOffer(baseUrl);
      const result = await merchantAction(baseUrl, transactionId, "accept", payload);
      assert.equal(result.response.status, 400);
    }
  });
});
...
test("public app-proxy paths do not expose merchant inbox or actions", async () => {
  await withServer(async ({ baseUrl }) => {
    ...
    const inbox = await fetch(`${baseUrl}/apps/counterpilot/merchant/offers?shop=counterpilot-dev.myshopify.com`);
...
test("offer intake validates amount, quantity, currency, product, store, and buyer contact", async () => {
  await withServer(async ({ baseUrl }) => {
    for (const payload of [
      ...
      validOffer({ buyer_email: "buyer@example.com", buyer_contact_token: "secret-token" })
    ]) {
      const response = await postJson(`${baseUrl}/counterpilot/offers`, payload);
      assert.equal(response.status, 400);
    }
...
test("app-proxy signature helper follows Shopify's sorted query-string signing shape", () => {
  ...
  const expectedMessage = "extra=1,2path_prefix=/apps/counterpilotshop=counterpilot-dev.myshopify.comtimestamp=1";
  const expected = crypto.createHmac("sha256", "secret").update(expectedMessage, "utf8").digest("hex");
